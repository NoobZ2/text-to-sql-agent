...
# 定义知识库标准文件名 (自适应)
KB_SCHEMA_FILE = "knowledge_base_schema.xlsx"
KB_QA_FILE = "knowledge_base_qa.xlsx"
...
with st.sidebar:
    ...
    if available_models:
        ...
        # st.stop()  # 移除 stop，确保后续知识库管理侧边栏可见
...
def init_knowledge_base(api_key,provider):
    ...
    if not os.path.exists(KB_SCHEMA_FILE) or not os.path.exists(KB_QA_FILE):
        return None, None

    schema_data = parse_schema_excel(KB_SCHEMA_FILE)
    qa_data = parse_qa_excel(KB_QA_FILE)
...
def get_sql_chain(retriever_schema, retriever_examples,provider, model_name, api_key):
    ...
    * [Check] 在编写代码的时候你需要在保持逻辑清晰的前提下，尽可能少的使用临时表和CTE结构，因为这会带来查询性能负担
...
with st.sidebar:
    ...
    # 显示文件状态
    has_files = os.path.exists(KB_SCHEMA_FILE) and os.path.exists(KB_QA_FILE)
    ...
    with st.expander("🛠️ 上传或更新知识库"):
        ...
        if schema_source == "上传 Excel":
            up_schema = st.file_uploader("上传 Schema Excel (任意文件名)", type=["xlsx"])
            if st.button("保存 Schema Excel") and up_schema:
                save_uploaded_file(up_schema, KB_SCHEMA_FILE)
                st.success("已保存!")
                st.rerun()
        else:
            ...
            if st.button("从 Sheets 同步 Schema") and s_id and s_sa:
                try:
                    ...
                    gsheet_to_excel_and_save(s_id, sa_info, KB_SCHEMA_FILE)
                    st.success("同步成功!")
                    st.rerun()
                except Exception as e:
                    st.error(str(e))
        ...
        if qa_source == "上传 Excel":
            up_qa = st.file_uploader("上传 问答样例 Excel (任意文件名)", type=["xlsx"])
            if st.button("保存样例 Excel") and up_qa:
                save_uploaded_file(up_qa, KB_QA_FILE)
                st.success("已保存!")
                st.rerun()
        else:
            ...
            if st.button("从 Sheets 同步样例") and q_id and q_sa:
                try:
                    ...
                    gsheet_to_excel_and_save(q_id, sa_info, KB_QA_FILE)
                    st.success("同步成功!")
                    st.rerun()
                except Exception as e:
                    st.error(str(e))
...
if has_files:
    ...
    if not target_model or not target_provider:
        st.warning("⚠️ 请先在左侧侧边栏配置模型 API Key 并选择模型。")
        st.stop()
    ...
    if rs:
        ...
        # 优化：模型缓存机制
        # ==========================================
        if "cached_chain" not in st.session_state:
            st.session_state.cached_chain = None
        if "cached_model_config" not in st.session_state:
            st.session_state.cached_model_config = {}
        ...
        # 仅在初次加载或配置发生变化时重新初始化 Chain
        if st.session_state.cached_chain is None or st.session_state.cached_model_config != current_config:
            with st.spinner(f"正在初始化模型链路 ({target_provider} - {target_model})..."):
                st.session_state.cached_chain = get_sql_chain(rs, re_, target_provider, target_model, api_key)
                st.session_state.cached_model_config = current_config

        chain = st.session_state.cached_chain
