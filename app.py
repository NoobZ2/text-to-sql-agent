...
with st.sidebar:
    ...
    # API Key 输入
    user_api_key = st.text_input("Google Gemini API Key", type="password", help="请输入您的 Google AI Studio API Key")
    
    # 模型选择
    model_options = ["gemini-3-pro-preview", "gemini-3-flash-preview"]
    selected_model = st.selectbox("选择模型版本", model_options, index=0)

    if not user_api_key:
        st.warning("⚠️ 请输入 API Key 以启动服务")
        st.stop()  # 如果没有 API Key，停止执行后续代码
    else:
        # 设置环境变量，供 LangChain 内部使用
...
def init_knowledge_base(api_key_trigger):
    # --- 内部解析函数 ---
...
# ==========================================
def get_sql_chain(retriever_schema, retriever_examples, model_name, api_key):
    # 动态初始化 LLM
...
if has_files:
    ...
    rs, re_ = init_knowledge_base(user_api_key)
    if rs:
        # 获取链对象，传入当前选择的模型和 API Key
        chain = get_sql_chain(rs, re_, selected_model, user_api_key)
    else:
